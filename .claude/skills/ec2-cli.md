---
name: ec2-cli
description: ec2, an advanced aws cli script that makes launching ec2 instances easy
---

# EC2-CLI Features Reference

## Commands

| Command | Description |
|---------|-------------|
| `ec2` | Interactive wizard to launch a new EC2 instance |
| `ec2 list` | List all running EC2 instances (ID, Name, DNS) |
| `ec2 term <instance-id>` | Terminate an instance (aliases: `terminate`, `kill`) |
| `ec2 <script.sh>` | Launch minimal instance, upload & execute script in background |

## Environment Variables

| Variable | Default | Description |
|----------|---------|-------------|
| `ACCEPT_DEFAULTS` | `false` | Skip prompts, use all defaults |
| `EC2_SECURITY_GROUP` | `SSH-HTTP-ICMP` | Security group name to use/create |
| `AWS_REGION` | from `aws configure` | Target AWS region |
| `AWS_AZ` | (empty) | Specific availability zone (optional) |
| `EC2_KEY_NAME` | `auto-ec2-<user>` | SSH key pair name in AWS |
| `EC2_KEY_FILE` | `~/.ssh/auto-ec2-<account>-<user>.pem` | Local SSH private key path |

## Interactive Prompts (when no args)

1. **Instance Type**: Specific type (e.g., `t3.medium`) OR `any` for auto-selection OR family prefix (e.g., `g5`)
2. **CPU Architecture**: `arm`/`arm64` (Graviton), `x86`/`x86_64`/`amd64`, or `any`
3. **vCPUs**: Number of virtual CPUs (default: 2)
4. **Memory**: RAM in GB (default: 1)
5. **Root Volume**: EBS size in GB (default: 4)
6. **Ephemeral Disk**: Local NVMe storage in GB (default: 0)
7. **GPUs**: Number of NVIDIA GPUs (default: 0)
   - If >0: GPU Model selection (e.g., `A10G`, `H100`, or `any`)
   - If `any`: GPU VRAM minimum in GB
8. **Operating System**: AMI pattern
   - `al202*-ami-*` = Amazon Linux 2023 (default)
   - `ubuntu/images/*ubuntu-*-24.04-*-server*` = Ubuntu 24.04
   - `Rocky-9-EC2-Base*` = Rocky Linux 9
9. **AWS Region**: Target region
10. **Instance Name**: Tag name and DNS hostname

## Supported Operating Systems

| OS | AMI Pattern | SSH User | AMI Owner |
|----|-------------|----------|-----------|
| Amazon Linux 2023 | `al202*-ami-*` | `ec2-user` | `137112412989` |
| Ubuntu | `ubuntu/images/*ubuntu-*-24.04-*-server*` | `ubuntu` | `099720109477` |
| Rocky Linux | `Rocky-9-EC2-Base*` | `rocky` | `792107900819` |

## Auto-Provisioning Features

- **AWS CLI**: Auto-installs v2 to `~/.local/bin` if missing
- **jq**: Auto-installs to `~/.local/bin` if missing
- **SSO Login**: Triggers `aws sso login --no-browser` if not authenticated
- **SSH Keys**: Creates ED25519 key pair or imports existing one
- **Security Group**: Creates with rules:
  - SSH (22): Your IP only
  - HTTP (80): Open
  - HTTPS (443): Open
  - ICMP: Open
- **Route 53 DNS**: Auto-registers `<name>.<domain>` if hosted zone exists
- **Cloud-init**: Uses `ec2-cloud-init.txt` if present (cwd or script dir)

## Instance Type Selection

When `INSTTYPE=any`, the script queries `describe-instance-types` with filters:
- Minimum vCPUs and memory
- CPU architecture match
- GPU count/model/VRAM requirements
- Ephemeral storage requirements
- Sorts by smallest matching type

Displays: Architecture, vCPUs, Memory, Ephemeral storage, GPU info, hourly price

## Script Upload Mode (`ec2 <file>`)

Creates a minimal instance (arm64, 2 vCPU, 1GB RAM, 4GB EBS) and:
1. Waits for SSH availability
2. SCPs the file to `/tmp/`
3. Executes in background with `nohup`
4. Detects shebang for direct execution vs `bash`

## Key Functions

| Function | Purpose |
|----------|---------|
| `list_instances` | Query and display running instances |
| `terminate_instance` | Send termination request |
| `check_aws` | Verify/install AWS CLI, authenticate, set key vars |
| `setup_ec2_key` | Create or import SSH key pair |
| `find_instance_type` | Query and select optimal instance type |
| `find_ami_image` | Find latest AMI matching OS/arch |
| `get_user_ip` | Fetch public IP for security group |
| `configure_security_group` | Create/reuse security group |
| `launch_instance` | Run EC2 instance with all parameters |
| `register_dns` | Create Route 53 A record |
| `wait_for_instance` | Poll until running with public IP |
...
# Launch with defaults (no prompts)
ACCEPT_DEFAULTS=true ec2

# List running instances
ec2 list

# Terminate instance
ec2 term i-0abc123def456

# Run script on ephemeral instance
ec2 my-job.sh

# GPU instance in specific region
AWS_REGION=us-west-2 ec2
# Then select: GPUS=1, GPU_MODEL=A10G

# Specific instance type
ec2
# Then enter: t3.xlarge
```

---

The script I am using is very comprehensive and supports: 
